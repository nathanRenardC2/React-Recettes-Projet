...
export default function Recette({ meal, id }) {
  ...
  useEffect(() => {
    ...
    localStorage.setItem('recettes', JSON.stringify(recettesSave));
  }, [recettesSave]);
  ...
  return (
    <div className="card bg-white dark:bg-slate-800 border-4 border-slate-300">
      ...
      <div>
        <svg onClick={saveRecette} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className={`w-7 h-7 self-end mr-5 mb-3 hover:cursor-pointer hover:text-red-500 dark:text-white transition ${isSave ? "text-red-500 dark:text-red-500" : ""}`}>
